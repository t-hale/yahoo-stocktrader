...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[sopapi] ", log.Ltime)
	}
	...
	{
		sopSvc = sopapi.NewSop(logger)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		sopEndpoints *sop.Endpoints
	)
	{
		sopEndpoints = sop.NewEndpoints(sopSvc)
	}
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, sopEndpoints, &wg, errc, logger, *dbgF)
		}

		{
			...
			handleGRPCServer(ctx, u, sopEndpoints, &wg, errc, logger, *dbgF)
		}
